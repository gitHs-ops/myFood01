...
## [2026-08-26] investigate | 동명이카테고리 메뉴 현황 재조사·기록
- 사용자 요청: 과거(2026-06-28) 세션에서 조사했던 "유사 단어 중복 메뉴"를 다시 보고해달라 함
- 과거 세션(local_71821160, 2026-06-28~07-16)을 뒤지는 대신 GET /api/menu/all(공개 API)로 현재 데이터를 직접 재조회해 같은 방식(이름 그룹핑)으로 재분석 — 재배포 불필요
- 결과: 순수 공백차이 중복은 0건(당시 문제는 해소된 듯). 대신 이름 완전동일·카테고리만 다른 10쌍 발견(2026-07-16에 이미 9건 알려져 있었음 + 신규 1건 두부두루치기)
- 사용자 결정: 병합 불필요 — 카테고리별 별개 상품으로 판단
- Pages updated: [[menu-warehouse]] ("동명이카테고리 메뉴 현황" 절 신설, 조사 이력·결정 기록)
---
