...
- 부가 문제(별건, 오픈 후 과제): 비밀번호와 API 토큰이 정적 HTML에 평문 하드코딩 — GitHub Pages 소스는 공개이므로 사실상 비밀번호가 공개된 상태. (2026-08-08: 값을 상수 `ONBAN_ADMIN_KEY` 한 곳으로 모으고 새 값으로 교체했으나, **공개 소스에 평문이라는 구조 자체는 그대로**다. 근본 해결은 아래 2단계.)
